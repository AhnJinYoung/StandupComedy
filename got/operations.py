...
class GraphOfOperations:
    ...
    def generate_followup_setup(self, state: GraphReasoningState, parent_id: str, num_samples: int = 2, style: str = "observational", category: str = "setup") -> List[str]:
        ...
        candidates = []
        path_text = state.get_path_contents(parent_id)
        prompt = self.prompter.generate_followup_setup_prompt(path_text, style=style, category=category)
        for _ in range(num_samples):
            response = self.llm.generate(prompt)
            if category == "incongruity":
                line = self.parser.parse_incongruity(response)
            else:
                line = self.parser.parse_setup(response)
    ...
    def generate_callback(self, state: GraphReasoningState, parent_id: str, num_samples: int = 3) -> List[str]:
        ...
        candidates = []
        path_contents = state.get_path_contents(parent_id)
        anchors = [c for c in path_contents if c != "Start of Standup Routine"]
        if not anchors:
            return candidates
        ...
        for i in range(num_samples):
            ...
            prompt = self.prompter.generate_callback_prompt(path_contents, anchor)
            response = self.llm.generate(prompt)
            callback_line = self.parser.parse_callback(response)
    ...
    def refine_node(self, state: GraphReasoningState, node_id: str, feedback: str = "", num_samples: int = 1) -> List[str]:
        ...
        for _ in range(num_samples):
            ...
            if node["type"] in {"setup", "incongruity"}:
                refined = self.parser.parse_setup(response)
                ...
                refined = self.parser.parse_punchline(response)
    ...
    def aggregate_nodes(self, state: GraphReasoningState, source_ids: List[str], target_parent: str, target_type: str = "setup") -> str:
        ...
        if target_type == "setup":
            aggregated = self.parser.parse_setup(response)
            if not self.validation.validate_setup(aggregated):
                return ""
        else:
            aggregated = self.parser.parse_punchline(response)
            if not self.validation.validate_punchline(aggregated):
                return ""
        return state.add_vertex(content=aggregated, type=target_type, parent_id=target_parent)
    ...
    def score_candidates(self, state: GraphReasoningState, candidate_ids: List[str]):
        ...
        for cid in candidate_ids:
            path_text = state.get_path_contents(cid)
            node = self.get_vertex_safe(state, cid)
            if not node:
                continue
            prompt = self.prompter.score_thought_prompt(path_text, node["type"])
            response = self.llm.generate(prompt)
            score_data = self.parser.parse_rich_score(response)
            if self.validation.validate_rich_score(score_data):
