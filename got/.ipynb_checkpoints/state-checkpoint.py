...
class GraphReasoningState:
    ...
    def __init__(self):
        ...
        self.root_id = "root"
        self.graph.add_node(self.root_id, content="Start of Standup Routine", type="root", score=0, reasoning="", pros="", cons="", next_action="branch_setup", metadata={})
    ...
    def add_vertex(self, content: str, type: str, parent_id: str = None, score: float = 0.0, metadata: Dict = None) -> str:
        ...
        self.graph.add_node(vertex_id, content=content, type=type, score=score, reasoning="", pros="", cons="", next_action="", metadata=metadata or {})
    ...
    def get_path_contents(self, vertex_id: str) -> List[str]:
        """Returns the textual content along the path root->vertex."""
        ids = self.get_path_to_root(vertex_id)
        return ["("+self.graph.nodes[i]["type"].lower() + ") " +self.graph.nodes[i]["content"] for i in ids]
