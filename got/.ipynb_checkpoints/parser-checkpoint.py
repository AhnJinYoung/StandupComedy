...
class Parser:
    ...
    def parse_category_decision(self, text: str) -> Dict[str, str]:
        ...
        # Allow trailing punctuation and extra words; take the first token after Category:
        category_match = re.search(r"Category:\s*([a-zA-Z_\-]+)", text)
        reason_match = re.search(r"Reason:\s*(.*)", text, re.DOTALL)
        category_raw = ""
        if category_match:
            category_raw = category_match.group(1).strip().lower().strip(" .,:;")
        else:
            # Fallback: look for any known category word in the text
            for cand in ["setup", "incongruity", "punchline", "callback", "end"]:
