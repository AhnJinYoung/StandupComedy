...
# Run baseline0 generative pipeline for one or more topics.
# Usage:
#   ./run_baseline0.sh "topic one" "topic two"
#   ./run_baseline0.sh 3   # runs 3 topics from TOPIC_POOL in order (cycles if N > pool size)
...
for topic in topics:
    print(f"=== Running baseline0 for topic: {topic} ===")
...
Script: [Your script here]""", max_new_tokens = 2048)
    ...
    # If generate returns a string, wrap in object for compatibility
    out = result if isinstance(result, str) else json.dumps(result)

    fname = f"outputs/baseline0_{topic.replace(' ', '_')}.json"
