...
# Run baseline2 generative pipeline for one or more topics.
# Usage:
#   ./run_baseline2.sh "topic one" "topic two"
#   ./run_baseline2.sh 3   # runs 3 random topics from TOPIC_POOL
...
for topic in topics:
    print(f"=== Running baseline2 for topic: {topic} ===")
...
, max_new_tokens = 2048)
    ...
    fname = f"outputs/baseline2_{topic.replace(' ', '_')}.json"
